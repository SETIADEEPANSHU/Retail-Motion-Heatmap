...
class MotionHeatmap:
    ...
    def __init__(
        ...
        sigma=1.5, # from 1.5 , with 5 lighter got more light
        # Scales intensity of the heatmap's red/blue tint: higher values will exaggerate the tinting
        color_intensity_factor= 7, # from 7 , 1 is for very light images overlap
    ...
    def generate_motion_heatmap(self, file_name='motion_heatmap.jpg'):
        ...
        for vertical_index, horizontal_index in itertools.product(range(self.num_vertical_divisions), range(self.num_horizontal_divisions)):
            ...
            for i, j in itertools.product(range(self.height//self.num_vertical_divisions), range(self.width//self.num_horizontal_divisions)):
                ...
                row = int(row)
                col = int(col)
                output_image[row][col][2] = self._clip_rgb(output_image[row][col][2] + offset) # blue green if not present
                #output_image[row][col][1] = self._clip_rgb(output_image[row][col][1] - offset) # yellowish if not present , changed from - to +
                output_image[row][col][0] = self._clip_rgb(output_image[row][col][0] - offset) # blue yellow if not present , changed from - to +
